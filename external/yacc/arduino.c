...
struct arduino_condition {
  ...
  int sig_value;
  char *operator;
  struct arduino_condition *right;
  struct arduino_condition *left;
};
...
Condition *single_condition(char *var, int signal) {
...
static void emit_condition(Condition *condition) {
    if (condition->left == NULL && condition->right == NULL) {
        printf("digitalRead(%s) == %s", condition->var_name , condition->sig_value ? "HIGH": "LOW");
    }  else {
        printf("((");
        emit_condition(condition->left);
        printf(")");
        char *op;
        ...
        printf(" %s (", op);
        emit_condition(condition->right);
        printf("))");
    }
}
